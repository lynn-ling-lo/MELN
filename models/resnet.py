...
class group_sa_layer(nn.Module):
    ...
    def __init__(self, channel, groups=4):
        ...
        self.cweight = nn.Parameter(torch.zeros(1, groups, channel // (2 * groups), 1, 1))
        self.cbias = nn.Parameter(torch.ones(1, groups, channel // (2 * groups), 1, 1))
        self.sweight = nn.Parameter(torch.zeros(1, groups, channel // (2 * groups), 1, 1))
    ...
    def forward(self, x):
        ...
        # spatial attention
        xs = self.gn(x_1.reshape(b * self.groups, -1, h, w)).reshape(b, self.groups, -1, h, w)
